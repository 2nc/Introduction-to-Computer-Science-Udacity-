...
def find_corespage(index,keyword):
    if keyword in index:
        return index[keyword]
    return []
def ranks_to_list(corespage,ranks):
    list = []
    for a in corespage:
        list.append([a,ranks[a]])
    return list
def quicksort(url_list):
    if len(url_list) == 0 or len(url_list) == 1:
        ...
        for i in range(1,len(url_list)):
            if url_list[i][1] > num:
                list_greater.append(url_list[i])
            else:
                list_lower.append(url_list[i])
        list_lower = quicksort(list_lower)
        list_greater = quicksort(list_greater)
        return list_lower + [url_list[0]] + list_greater
def ordered_search(index, ranks, keyword):
    corespage = find_corespage(index,keyword)
    if corespage == []:
        return None
    else:
        url_list = ranks_to_list(corespage,ranks)
        sort_list = quicksort(url_list)
    list = []
    for a in range(len(sort_list)-1,-1,-1):
        list.append(sort_list[a][0])
    return list
