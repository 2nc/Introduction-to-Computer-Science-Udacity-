...
def is_reciprocal(graph,page,page_to,k):
    if k == 0:
        if page == page_to:
            return True
        else:
            return False
    if k == 1:
        if page_to in graph[page]:
            return True
        else:
            return False
    for page_sub in graph[page]:
        if is_reciprocal(graph,page_sub,page_to,k - 1):
            return True
    return False
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    if not is_reciprocal(graph,page,node,k):
