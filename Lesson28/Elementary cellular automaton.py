...
def pattern_to_next_table(pat):
    ...
    while num > 0:
        ...
        num /= 2
    return table
def produce_next_gen(string,pre,table):
    string_out = ""
    if len(string) >= 3:
        str_cal = string[-1] + string[0:2]
        string_out += table[pre.index(str_cal)]
        for i in range(1,len(string) - 1):
            str_cal = string[i-1:i+2]
            string_out += table[pre.index(str_cal)]
        str_cal = string[len(string) - 2 :] + string[0]
        string_out += table[pre.index(str_cal)]
    if len(string) == 1:
        str_cal = string + string + string
        string_out += table[pre.index(str_cal)]
    if len(string) == 2:
        str_cal = string[-1] + string[0] + string[1]
        string_out += table[pre.index(str_cal)]
        str_cal = string[0] + string[1] + string[0]
        string_out += table[pre.index(str_cal)]
    return string_out
    
def cellular_automaton(string, pattern, gen):
    pre = ['xxx', 'xx.', 'x.x', 'x..', '.xx', '.x.', '..x', '...']
    next_table = pattern_to_next_table(pattern)
    #print next_table
    next_string = string
    for i in range(1,gen + 1):
        next_string = produce_next_gen(next_string,pre,next_table)
    return next_string
