...
testfs_read_block(struct inode *in, int log_block_nr, char *block)
{
	...
	if (log_block_nr < NR_DIRECT_BLOCKS) {
		...
		if (log_block_nr >= NR_INDIRECT_BLOCKS){	//find block in double block
...
testfs_read_data(struct inode *in, char *buf, off_t start, size_t size)
{
	...
	assert(buf);
	if (start + (off_t) size > in->in.i_size) {	//over the size of i_size
		size = in->in.i_size - start;
	}
	if (block_ix + size > BLOCK_SIZE) {	//over the block_size
		...
		while(size - written > BLOCK_SIZE){
			...
			if ((ret = testfs_read_block(in, block_nr, block)) < 0)
				return ret;
			memcpy(buf + written, block + block_ix, BLOCK_SIZE);	//block_ix=0
			written += BLOCK_SIZE;
			block_nr++;
			block_ix = 0;
		}	
	...
	if ((ret = testfs_read_block(in, block_nr, block)) < 0)
		return ret;
	memcpy(buf, block + block_ix, size);	//why still read from buf?
...
testfs_allocate_block(struct inode *in, int log_block_nr, char *block)
{
	...
	assert(((int *)indirect)[log_block_nr] == 0);	//first read indirect block then allocate
	phy_block_nr = testfs_alloc_block_for_inode(in);
